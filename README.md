...
stateDiagram-v2
    [*] --> Splash
    Splash --> Portfolio: five-second reveal completes
    Portfolio --> LightMode: visitor chooses light
    Portfolio --> DarkMode: visitor chooses dark
    LightMode --> Section: scroll or navigation
    DarkMode --> Section: scroll or navigation
    Section --> ProjectModal: open a project
    ProjectModal --> Section: close details
    Section --> Contact: continue the story
```

The experience is deliberately cinematic: `splash.html` hands off to `index.html`; CSS owns the red/gold visual system; JavaScript coordinates theme persistence, navigation state, scroll reveals, skill tabs, project modals, and form feedback.
...
Source code is available under the repository’s [MIT License](LICENSE). Personal profile content, photographs, résumés, institutional marks, and third-party assets remain the property of their respective rights holders.
